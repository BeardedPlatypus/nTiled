...
// ----------------------------------------------------------------------------
SCENARIO("getFurthestNodeBehaviour should throw a SLTOutOfLatticeBoundsException when a cube is returned which does not partly cover the Lattice",
  ...
  GIVEN("A series of lattices") {
    ...
    WHEN("The upperbound of the cube is smaller than lattice origin") {
      ...
      THEN("getFurthestNodeBehaviour should throw a SLTOutOfLatticeBoundsException") {
        REQUIRE_THROWS_AS(lattice_1->getFurthestNodeAwayFromLightSource(lowerbound_1, 1.0),
                       nTiled::pipeline::hashed::SLTOutOfLatticeBoundsException);
        REQUIRE_THROWS_AS(lattice_2->getFurthestNodeAwayFromLightSource(lowerbound_1, 1.0),
                       nTiled::pipeline::hashed::SLTOutOfLatticeBoundsException);
        REQUIRE_THROWS_AS(lattice_3->getFurthestNodeAwayFromLightSource(lowerbound_1, 1.0),
                       nTiled::pipeline::hashed::SLTOutOfLatticeBoundsException);

        REQUIRE_THROWS_AS(lattice_1->getFurthestNodeAwayFromLightSource(lowerbound_2, 1.0),
                       nTiled::pipeline::hashed::SLTOutOfLatticeBoundsException);
        REQUIRE_THROWS_AS(lattice_2->getFurthestNodeAwayFromLightSource(lowerbound_2, 1.0),
                       nTiled::pipeline::hashed::SLTOutOfLatticeBoundsException);
        REQUIRE_THROWS_AS(lattice_3->getFurthestNodeAwayFromLightSource(lowerbound_2, 1.0),
                       nTiled::pipeline::hashed::SLTOutOfLatticeBoundsException);

        REQUIRE_THROWS_AS(lattice_1->getFurthestNodeAwayFromLightSource(lowerbound_3, 1.0),
                       nTiled::pipeline::hashed::SLTOutOfLatticeBoundsException);
        REQUIRE_THROWS_AS(lattice_2->getFurthestNodeAwayFromLightSource(lowerbound_3, 1.0),
                       nTiled::pipeline::hashed::SLTOutOfLatticeBoundsException);
        REQUIRE_THROWS_AS(lattice_3->getFurthestNodeAwayFromLightSource(lowerbound_3, 1.0),
                       nTiled::pipeline::hashed::SLTOutOfLatticeBoundsException);

        REQUIRE_THROWS_AS(lattice_1->getFurthestNodeAwayFromLightSource(lowerbound_4, 1.0),
                       nTiled::pipeline::hashed::SLTOutOfLatticeBoundsException);
        REQUIRE_THROWS_AS(lattice_2->getFurthestNodeAwayFromLightSource(lowerbound_4, 1.0),
                       nTiled::pipeline::hashed::SLTOutOfLatticeBoundsException);
        REQUIRE_THROWS_AS(lattice_3->getFurthestNodeAwayFromLightSource(lowerbound_4, 1.0),
                       nTiled::pipeline::hashed::SLTOutOfLatticeBoundsException);
      }
    }
...
SCENARIO("getFurthestNodeBehaviour should return the value of the node furthest to the light source when provided with a correct cube",
         "[LightOctreeFull][SLTFull][Lattice]") {
  GIVEN("a SLTBuilder and a set of lattices") {
          ...
          for (unsigned int x_i = 0; x_i < indices.at(i); x_i++) {
            for (unsigned int y_i = 0; y_i < indices.at(i); y_i++) {
              for (unsigned int z_i = 0; z_i < indices.at(i); z_i++) {
                ...
                result = lattices.at(i)->getFurthestNodeAwayFromLightSource(cube_origin, 0.2);
    ...
    WHEN("A cube consisting of multiple nodes is provided") {
      glm::vec3 octree_origin = builder.getOriginOctree();
      glm::uvec3 result;
      THEN("The value of the node furthest to the origin is returned") {
        for (unsigned int i = 0; i < 20; i++) {
          for (unsigned int j = 1; j <= unsigned int(floor(indices.at(i) * 0.5)); j++) {
            result = lattices.at(i)->getFurthestNodeAwayFromLightSource(lattices.at(i)->getOrigin(), 
                                                                 j * builder.getMinimalNodeSize());
            REQUIRE(result.x == 0);
            REQUIRE(result.y == 0);
            REQUIRE(result.z == 0);
          }
    ...
    WHEN("A cube of the width and origin similar to the lattice is provided") {
      ...
      THEN("Then the furthest node should be returned") {
        for (unsigned int i = 0; i < 20; i++) {
          result = lattices.at(i)->getFurthestNodeAwayFromLightSource(lattices.at(i)->getOrigin(), lattices.at(i)->getWidth());
          n_nodes = lattices.at(i)->getNumberNodesDim();


          if (result.x > 0)
            REQUIRE(result.x == n_nodes - 1);
          else
            REQUIRE(result.x == 0);

          if (result.y > 0)
            REQUIRE(result.y == n_nodes - 1);
          else
            REQUIRE(result.y == 0);

          if (result.z > 0)
            REQUIRE(result.z == n_nodes - 1);
          else
            REQUIRE(result.z == 0);
    ...
    WHEN("A cube partly covering the lattice is provided") {
      glm::vec3 octree_origin = builder.getOriginOctree();
      glm::uvec3 result;
      THEN("The value of the node furthest from the the origin is returned") {
        for (unsigned int i = 0; i < 20; i++) {
          for (unsigned int j = 1; j <= unsigned int(floor(indices.at(i) * 0.5)); j++) {
            result = lattices.at(i)->getFurthestNodeAwayFromLightSource(lattices.at(i)->getOrigin() - glm::vec3(1.0), 
    ...
    WHEN("A cube completely covers the lattice") {
      ...
      glm::uvec3 result;
      unsigned int n_nodes;
      THEN("The value of the furthest node should be returned") {
        for (unsigned int i = 0; i < 20; i++) {
          result = lattices.at(i)->getFurthestNodeAwayFromLightSource(lattices.at(i)->getOrigin() - glm::vec3(3.0), lattices.at(i)->getWidth() + 6.0);
          n_nodes = lattices.at(i)->getNumberNodesDim();
